...
export default function PurchaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statusBadge = (
    <span className={`text-xs px-2.5 py-1 rounded-full font-medium shrink-0 ${purchase.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
    ...
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="print:hidden">
        ...
        <div className="flex items-center gap-2 md:hidden">
          <Link href="/purchases"><Button variant="ghost" size="sm" className="shrink-0"><ArrowLeft size={16} /></Button></Link>
          <h1 className="text-lg font-bold text-gray-900 truncate">Đơn mua #{purchase.id}</h1>
          {statusBadge}
        </div>
      ...
      <PrintLayout
        title={`ĐƠN MUA VÀO #${purchase.id}`}
        subtitle={`Ngày: ${purchase.date}${purchase.time ? ' — ' + purchase.time.slice(0, 5) : ''} | Người tạo: ${purchase.created_by?.name ?? ''}`}
      >
        {/* ── Info section ── */}
        {/* Mobile: single card */}
        <Card className="md:hidden">
          <div className="space-y-0">
            <InfoRow label="Người bán">{purchase.customer?.name}</InfoRow>
            {purchase.customer?.phone && <InfoRow label="SĐT">{purchase.customer.phone}</InfoRow>}
            {purchase.customer?.address && <InfoRow label="Địa chỉ">{purchase.customer.address}</InfoRow>}
            ...
            <InfoRow label="Người tạo">{purchase.created_by?.name}</InfoRow>
            {purchase.note && <InfoRow label="Ghi chú">{purchase.note}</InfoRow>}
          </div>
        </Card>
            ...
            <div className="space-y-2 text-sm">
              ...
              {purchase.customer?.address && (
                <div className="flex justify-between">
                  ...
                  <span className="text-right">{purchase.customer.address}</span>
                </div>
              )}
            </div>
          </Card>
        ...
        {/* ── Items ── */}
        <Card title="Chi tiết hàng hoá" className="print-no-break mt-4">
          {/* Desktop table */}
          ...
          <div className="md:hidden space-y-2">
            ...
            <div className="flex items-center justify-between pt-3 border-t border-gray-200 px-1">
              ...
              <span className="text-xl font-bold text-emerald-600">{fmt(purchase.total_amount)}</span>
            </div>
          </div>
        </Card>
      </PrintLayout>
