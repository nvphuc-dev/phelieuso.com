...
import { purchaseService, PurchaseItemInput } from '@/services/purchase.service';
...
export default function NewPurchasePage() {
  ...
  const [items, setItems] = useState<ItemRow[]>([
    { _key: keySeq++, material_id: 0, weight: 0, price_per_unit: 0 },
  ]);
  ...
  const { data: customers = [] } = useQuery({ queryKey: ['customers', 'seller'], queryFn: () => customerService.getAll({ type: 'seller' }) });
  ...
  const updateRow = (key: number, field: keyof PurchaseItemInput, value: unknown) =>
    setItems((p) => p.map((i) => {
      if (i._key !== key) return i;
      const updated = { ...i, [field]: value };
      if (field === 'material_id') {
        const mat = materials.find((m) => m.id === Number(value));
        if (mat?.default_purchase_price) updated.price_per_unit = Number(mat.default_purchase_price);
      }
      return updated;
    }));

  const total = items.reduce((s, i) => s + (i.weight || 0) * (i.price_per_unit || 0), 0);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!customerId) { setError('Chọn người bán'); return; }
