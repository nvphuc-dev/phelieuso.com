  ...
  return (
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl md:text-2xl font-bold text-gray-900">Tạo đơn bán ra</h1>
      </div>
                  ...
                  <Select value={row.material_id || ''} onChange={(e) => updateRow(row._key, 'material_id', Number(e.target.value))}>
                    ...
                    {materials.map((m) => (
                      <option key={m.id} value={m.id}>
                        {m.name} (tồn: {new Intl.NumberFormat('vi-VN').format(getStock(m.id))} kg)
                      </option>
                    ))}
                  </Select>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Input label="Cân nặng (kg)" type="number" step="0.001" min="0"
                        value={row.weight || ''} onChange={(e) => updateRow(row._key, 'weight', e.target.value)}
                        className={overStock ? 'border-red-400' : ''} placeholder="0" />
                      {overStock && <p className="text-xs text-red-500 mt-0.5">Vượt ({stock} kg)</p>}
                    </div>
